 ...
 * ARBITER_ACCEPT, ARBITER_VOTE) are covered in dispute-sig-verification.test.js (PR #43).
 */
...
/**
 * Helper: create a proposal via the client API (which signs correctly)
 * and return its id. Used to set up state for ACCEPT/REJECT/COMPLETE/DISPUTE tests.
 */
async function createAcceptedProposal(alice, bob, aliceClient, bobClient) {
  const proposal = await aliceClient.propose(bobClient.agentId, {
    task: 'Setup proposal',
    amount: 10,
    currency: 'TEST',
    expires: 300,
  });
  const acceptPromise = waitForMessage(aliceClient, 'ACCEPT');
  await bobClient.accept(proposal.id);
  await acceptPromise;
  return proposal.id;
}
...
describe('Proposal Signature Verification', () => {
  ...
  it('rejects accept with wrong signature', async () => {
    ...
    try {
      ...
      // Sign different content to produce a valid but wrong signature
  ...
  it('accepts complete with valid signature', async () => {
    ...
    try {
      ...
      // Create and accept a proposal first
      const proposal = await alice.propose(bob.agentId, {
      ...
      // Complete with valid signature via client API
  ...
  it('accepts dispute with valid signature', async () => {
    ...
    try {
      ...
      // Dispute with valid signature via client API
