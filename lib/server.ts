...
export class AgentChatServer {
  ...
  _log(event: string, data: Record<string, unknown> = {}): void {
    ...
    if ((entry as any).error instanceof Error) {
      (entry as any).error = { message: (entry as any).error.message, stack: (entry as any).error.stack };
    }
    try {
  ...
  _handleMessage(ws: ExtendedWebSocket, data: string): void {
    ...
    if (!result.valid) {
      const errorStr = (result as { valid: false; error: string }).error;
