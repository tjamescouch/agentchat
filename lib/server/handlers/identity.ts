...
export function handleIdentify(server: AgentChatServer, ws: ExtendedWebSocket, msg: IdentifyMessage): void {
  ...
  if (msg.pubkey) {
    ...
    // Correlation trace id for this auth flow
    const traceId = `${ws._connId || 'conn'}:${challengeId}`;
...
export function handleVerifyIdentity(server: AgentChatServer, ws: ExtendedWebSocket, msg: VerifyIdentityMessage): void {
  ...
  let verified = false;
  try {
    verified = Identity.verify(expectedContent, msg.signature, challenge.pubkey);
  } catch (err) {
    // Treat exceptions as verification failures but log the error
    server._log('challenge_verify_error', {
  ...
  server._log('challenge_verified', {
    challengeId: msg.challenge_id,
    verified: !!verified,
